...
CPPFLAGS="$CPPFLAGS -DSHOW_PCMVOL_BAR=1"
...
$SRCDIR/configure \
  ...
  --enable-decoder="MP3 MP3ADU MP3ON4 MP2  AC3 AAC" \
  --enable-decoder="ADPCM_IMA_WAV ADPCM_MS PCM_S16LE" \
  --disable-encoder=all \
  --enable-encoder="PNG" \
  --disable-parser=all \
  --enable-parser="H264 MPEG4VIDEO  MPEGAUDIO AAC AC3" \
  --disable-demuxer=all \
  --enable-demuxer="MOV AVI  MP3 WAV PCM_S16LE AAC AC3" \
  --enable-demuxer="MPEGTS MPEGTSRAW MPEGPS H264  RAWVIDEO  IMAGE2" \
  --disable-muxer=all \
  --enable-muxer=none \
